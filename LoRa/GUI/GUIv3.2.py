...
ser = serial.Serial('COM7')  # go to device manager and enter in the COM port desired
if DEBUG_ON:
...
def windowSetup():
    ...
    headers = ['iTemp', 'eTemp', 'X', 'Y', 'Z', 'pressure', 'time', 'GPS coord', 'GPS coord', 'Alt','Pressure-Temperature','Packets Recieved']
    if DEBUG_ON:
        print("opening csv")
