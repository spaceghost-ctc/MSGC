...
#  TODO: Board ID Print Terminal <2021.8.5> -L [Modified to print based on thread call]
...
class Window(object):
    def __init__(self):
        ...
        self.nemo_total = None
        self.nemo_total2 = None
    ...
    def setup_nemo(self):
        ...
        nemo_total_label = Label(self.frame, text="Counter Total")
        nemo_total_label.grid(row=28, column=1)
        self.nemo_total = Label(self.frame, text="")
        self.nemo_total.grid(row=27, column=2)
        return
    ...
    def setup_nemo2(self):
        ...
        nemo_total_label2 = Label(self.frame, text="Counter Total")
        nemo_total_label2.grid(row=28, column=1)
        self.nemo_total2 = Label(self.frame, text="")
        self.nemo_total2.grid(row=27, column=2)
        return
    ...
    def system_running(self):
        while ui.running:
            ...
            if test == '[':
                ...
                self.data_output = np.asarray(string_rec.split(','))
                self.csvwriter.writerow(self.data_output)
                nemo_sub = self.data_output[1:-2]
                for i in nemo_sub: nemo_tot = nemo_tot + i
                ui.nemo_total["text"] = nemo_tot
            else:
                print("TEST FAILED")
    ...
    def system_running(self):
        ...
        while ui.running2:
            ...
            if test == '[':
                ...
                ui.lat_val2["text"] = str("%.4f" % self.sensor_data[7])
                ui.lon_val2["text"] = str("%.4f" % self.sensor_data[8])
                ui.altm_val2["text"] = str("%.4f" % self.sensor_data[9])
                ui.altft_val2["text"] = str(("%.4f" % self.sensor_data[9].astype(int) * 3.28084))
                ui.temp1_val2["text"] = str("%.4f" % self.sensor_data[0])
                ui.temp2_val2["text"] = str("%.4f" % self.sensor_data[1])
                ui.tempp_val2["text"] = str("%.4f" % self.sensor_data[10])
                ui.press_val2["text"] = str("%.4f" % self.sensor_data[5])
                ui.x_val2["text"] = str("%.4f" % self.sensor_data[2])
                ui.y_val2["text"] = str("%.4f" % self.sensor_data[3])
                ui.z_val2["text"] = str("%.4f" % self.sensor_data[4])
