...
int main(void)
{
    ...
    //-------SPI-------
    P5DIR |= BIT3;  // LED Power-on (Pin 40 P5.3)
    P5OUT &= ~BIT3; //set LED on when powered
    ...
    //-------UART--------
    P5DIR |= BIT0; // LED Power-on (Pin 43 P5.0)
    P5OUT |= BIT0; //set LED off when powered
    ...
    SPI_tx(FR_MSB_06, 0xF0); //Set to 912 MHz MSB
